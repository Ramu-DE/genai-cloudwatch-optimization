#!/usr/bin/env python3
"""
Max Training Agent - Complete Implementation with Memory & Session Management
...
log_file = os.path.join(os.path.expanduser('~'), 'max_agent.log')
logging.basicConfig(
    level=logging.INFO,
...
logger.info("🏋️ MAX TRAINING AGENT STARTING UP - FULL IMPLEMENTATION")
...
def sanitize_session_id(session_id):
    """Sanitize session ID to match AgentCore requirements: [a-zA-Z0-9][a-zA-Z0-9-_]*"""
    if not session_id:
        return "default"
    
    # Remove any invalid characters and ensure it starts with alphanumeric
    import re
    # Replace invalid characters with underscores
    sanitized = re.sub(r'[^a-zA-Z0-9\-_]', '_', str(session_id))
    
    # Ensure it starts with alphanumeric character
    if not sanitized or not sanitized[0].isalnum():
        sanitized = f"s_{sanitized}" if sanitized else "default"
...
def initialize_memory(customer_id=None, session_id=None):
    ...
    if customer_id:
        target_memory_id = f"MaxTraining_Customer_{customer_id}"
    elif session_id and 'cust_' in session_id:
        cust_part = session_id.split('cust_')[1].split('-')[0] if 'cust_' in session_id else None
        target_memory_id = f"MaxTraining_Customer_{cust_part}" if cust_part else f"MaxTraining_Session_{session_id[:20]}"
    else:
        target_memory_id = f"MaxTraining_Session_{session_id[:20] if session_id else 'default'}"
    
    # Check if we already have a memory_id for this customer
    if memory_id:
        logger.info(f"🧠 Using cached memory ID for max/{customer_id}")
        return memory_id
    ...
    try:
        ...
        try:
            memory = memory_client.create_memory_and_wait(
                ...
                description="Persistent memory for Max training agent with long-term retention",
                strategies=[
                    {
                        "summaryMemoryStrategy": {
                            "name": "TrainingSessionSummarizer",
                            "description": "Summarizes training sessions and behavioral progress",
                            "namespaces": ["/summaries/max/{actorId}/{sessionId}"]
                        }
                    },
                    {
                        "semanticMemoryStrategy": {
                            "name": "TrainingKnowledgeBase",
                            "description": "Stores training techniques, behavioral knowledge, and obedience information",
                            "namespaces": ["/knowledge/max/{actorId}/training"]
                        }
                    },
                    {
                        "userPreferenceMemoryStrategy": {
                            "name": "TrainingPreferences",
                            "description": "Learns customer training preferences and pet behavioral patterns",
                            "namespaces": ["/preferences/max/{actorId}"]
                        }
                    }
                ],
...
def initialize_memory_tools(customer_id, session_id=None):
    ...
    try:
        ...
        namespace = f"/preferences/max/{customer_id}"
...
def save_conversation_to_memory(customer_id, session_id, user_message, max_response):
    ...
    try:
        logger.info(f"💾 Saving to memory: User='{user_message[:30]}...', Max='{max_response[:30]}...'")
        ...
        truncated_max_response = max_response
        if len(max_response) > max_length:
            # Intelligent truncation - keep beginning and end
            half_length = max_length // 2
            truncated_max_response = (
                max_response[:half_length] + 
                "... [middle truncated] ..." + 
                max_response[-half_length:]
            )
            logger.warning(f"⚠️ Max response truncated from {len(max_response)} to {len(truncated_max_response)} chars")
        ...
        memory_client.create_event(
            ...
            messages=[
                ...
                (truncated_max_response, "assistant")
            ]
        )
...
def extract_conversation_context(user_input):
    ...
    services = ["training", "obedience", "behavioral", "consultation", "puppy training", "leash training", "advanced training", "behavioral modification"]
...
def get_enhanced_memory_context(customer_id, query="recent conversations and preferences"):
    ...
    try:
        ...
        try:
            pref_namespace = f"/preferences/max/{customer_id}"
            ...
            pref_memories = memory_client.retrieve_memories(
                ...
                query="training preferences and behavioral patterns"
            )
            if pref_memories:
                context_parts.append("Training Preferences:")
                for memory in pref_memories[:2]:
                    content = memory.get('content', '')
                    if content:
                        context_parts.append(f"- {content}")
                logger.debug(f"✅ Found {len(pref_memories)} training preference memories")
        except Exception as e:
            logger.debug(f"Could not retrieve preferences: {e}")
        
        # 2. Get training knowledge/facts
        try:
            knowledge_namespace = f"/knowledge/max/{customer_id}/training"
            ...
            knowledge_memories = memory_client.retrieve_memories(
                ...
                query="pet training techniques and behavioral information"
            )
            if knowledge_memories:
                context_parts.append("Training Knowledge:")
                for memory in knowledge_memories[:2]:
                    content = memory.get('content', '')
                    if content:
                        context_parts.append(f"- {content}")
                logger.debug(f"✅ Found {len(knowledge_memories)} training knowledge memories")
        except Exception as e:
        ...
        try:
            ...
            summary_namespace = f"/summaries/max/{customer_id}"
            logger.debug(f"🔍 Searching summaries in namespace: {summary_namespace}")
            summary_memories = memory_client.retrieve_memories(
                memory_id=memory_id,
                namespace=summary_namespace,
                query="recent training sessions and behavioral assessments"
            )
            if summary_memories:
                context_parts.append("Recent Training Sessions:")
        ...
        if context_parts:
            context = "Long-term training memory context:\n" + "\n".join(context_parts)
            logger.info(f"✅ Retrieved long-term training memory context: {len(context)} characters")
            return context
        else:
            logger.debug("🔍 No long-term memories found, trying fallback approach...")
            
            # Fallback: Try broader namespaces
            for alt_namespace in [f"/preferences/max/{customer_id}", f"/knowledge/max/{customer_id}", f"/summaries/max/{customer_id}"]:
                try:
                ...
                if 'payload' in event:
                    for payload_item in event['payload']:
                        if 'conversational' in payload_item:
                            ...
                            role = "Customer" if conv.get('role', '').lower() == 'user' else "Max"
...
def save_appointment_with_logging(appointment_data):
    ...
    try:
        ...
        dynamodb_item = {
            ...
            'serviceType': {'S': appointment_data.get('service_type', 'training consultation')},
            'agentType': {'S': 'max'},
            'status': {'S': 'scheduled'},
...
def training_services():
    """Get available training services with detailed descriptions"""
    return """🎾 Available Training Services:

� Basic Obedience Training - $60-80 (60 minutes)
...
🏠 Puppy Training Package - $75-95 (75 minutes)
- House training and crate training
- Socialization with people and pets
- Bite inhibition and gentle play
- Basic commands and manners
- Puppy-proofing guidance

🧠 Behavioral Modification - $85-120 (90 minutes)
...
� FGroup Training Classes - $45-65 (60 minutes)
...
🎯 Advanced Training - $70-100 (75 minutes)
- Complex commands and tricks
- Off-leash reliability
- Competition preparation
- Specialized skills training
- Performance and agility basics

🚶 Leash Training Intensive - $55-75 (45 minutes)
- Loose leash walking
...
All training uses positive reinforcement methods and includes take-home practice guides."""

@tool
def behavioral_assessment(behavioral_issues: str, pet_name: str = None, severity: str = "moderate"):
    """Assess behavioral issues and provide training recommendations"""
    
    # Categorize behavioral issues
    aggression_keywords = ['aggressive', 'biting', 'snapping', 'growling', 'attacking']
    anxiety_keywords = ['anxious', 'fearful', 'scared', 'separation anxiety', 'nervous']
    obedience_keywords = ['won\'t listen', 'doesn\'t come', 'pulls on leash', 'jumping', 'not house trained']
    
    issues_lower = behavioral_issues.lower()
    
    if any(keyword in issues_lower for keyword in aggression_keywords):
        issue_type = "AGGRESSION"
        urgency = "HIGH"
        recommendation = "🚨 Behavioral Modification sessions recommended immediately. Safety is our priority."
    elif any(keyword in issues_lower for keyword in anxiety_keywords):
        issue_type = "ANXIETY/FEAR"
        urgency = "MODERATE"
        recommendation = "🧠 Behavioral Modification with gradual desensitization approach."
    elif any(keyword in issues_lower for keyword in obedience_keywords):
        issue_type = "OBEDIENCE"
        urgency = "LOW"
        recommendation = "🎾 Basic Obedience Training or Group Classes would be beneficial."
    else:
        issue_type = "GENERAL"
        urgency = "LOW"
        recommendation = "📋 Assessment needed to determine best training approach."
    
    assessment = f"""🎾 Training Behavioral Assessment

📋 Reported Issues: {behavioral_issues}
🏷️ Issue Category: {issue_type}
⚠️ Priority Level: {urgency}
...
- Behavioral modification: 8-12 weeks
- Maintenance: Ongoing practice

⚠️ This is a preliminary assessment. In-person evaluation recommended for comprehensive training plan."""

    return assessment

@tool
def modify_training_appointment(customer_id: str, pet_name: str, old_date: str, new_date: str, new_time: str = None, new_service: str = None):
    """Modify an existing training session appointment"""
    try:
        logger.info(f"🔄 Modifying training appointment for {customer_id}, pet: {pet_name}")
...
@tool
def schedule_training_appointment(customer_id: str, pet_name: str, service_type: str, preferred_date: str, preferred_time: str = "2:00 PM", notes: str = ""):
    """Schedule a training session appointment with proper date/time validation"""
    try:
...
Would you like me to suggest some available appointment slots?"""
        ...
        appointment_data = {
            ...
            'notes': notes,
            'provider': 'Max - Training Specialist'
        }
        
        # Determine cost and duration based on service type
        service_costs = {
            'Basic Obedience Training': {'cost': 70, 'duration': 60},
            'Puppy Training Package': {'cost': 85, 'duration': 75},
            'Behavioral Modification': {'cost': 105, 'duration': 90},
            'Group Training Classes': {'cost': 55, 'duration': 60},
            'Advanced Training': {'cost': 85, 'duration': 75},
            'Leash Training Intensive': {'cost': 65, 'duration': 45}
        }
        ...
        if save_appointment_with_logging(appointment_data):
            ...
            return f"""✅ Training Session Scheduled Successfully!
...
- Trainer: Max (Training Specialist)

📝 Notes: {notes if notes else 'None'}

💡 Preparation Instructions:
- Bring current collar and leash
- High-value treats for motivation
- Any training tools currently used
- List of current commands known
- Behavioral concerns or goals

🎾 Training Philosophy: We use positive reinforcement methods to build confidence and strengthen the bond between you and your pet."""
        else:
            return f"❌ Error scheduling appointment. Please try again or contact us directly."
        
    except Exception as e:
        logger.error(f"Error scheduling training appointment: {e}")
...
@tool
def save_conversation_memory(customer_id: str, user_message: str, max_response: str, session_id: str = None):
    ...
    try:
        ...
        save_conversation_to_memory(customer_id, session_id or "default", user_message, max_response)
...
# Create base tools list (training-specific tools)
base_tools = [
    get_customer_and_pet_info,
    training_services,
    behavioral_assessment,
    schedule_training_appointment,
    modify_training_appointment,
...
agent = Agent(
    model=model,
    tools=base_tools,
    system_prompt="""You are Max, an experienced and patient pet training specialist at GenAI Pet Store.

CRITICAL: You MUST use your tools to provide accurate information. Never guess or make up information.

🔧 **MANDATORY Tool Usage Rules:**
1. When a customer provides a customer ID or asks about their pets/training needs, ALWAYS use get_customer_and_pet_info tool FIRST
2. When asked about training services, pricing, or behavioral programs, ALWAYS use training_services tool
3. When asked to assess behavioral concerns, ALWAYS use behavioral_assessment tool
4. When asked to schedule training sessions, ALWAYS use schedule_training_appointment tool
5. NEVER provide customer or pet behavioral information without using the tools first

🎾 **Your Available Tools:**
- get_customer_and_pet_info(customer_id, pet_name=None): Look up customer and pet data including behavioral information
- training_services(): Get current training services, pricing, and behavioral programs
- behavioral_assessment(behavioral_issues, pet_name=None, severity=""): Assess behavioral needs and provide recommendations
- schedule_training_appointment(customer_id, pet_name, service_type, preferred_date, notes=""): Book NEW training sessions
- modify_training_appointment(customer_id, pet_name, old_date, new_date, new_time=None, new_service=None): MODIFY existing training appointments
...
- save_conversation_memory(customer_id, user_message, max_response, session_id=None): Save conversation context

🎯 **Critical Response Patterns:**
1. Customer asks about pets/behavioral info → IMMEDIATELY use get_customer_and_pet_info tool
2. Customer asks about training services/pricing → IMMEDIATELY use training_services tool
3. Customer mentions behavioral concerns → IMMEDIATELY use behavioral_assessment tool
4. Customer wants to book training consultation → Use schedule_training_appointment tool

🚨 **TRAINING FOCUS AREAS:**
- Basic obedience (sit, stay, come, heel)
- Behavioral modification (aggression, anxiety, fear)
...
- Assess specific behavioral issues or training goals from the database
- Provide positive reinforcement techniques and clear instructions
- Reference pet's current training level and temperament from the database
- Recommend appropriate training programs based on pet's profile
- Be encouraging, patient, and solution-focused in your recommendations

🚨 **TRAINING CONSULTATION REQUIREMENTS:**
- For NEW consultations: Use schedule_training_appointment tool
- For CHANGING/MODIFYING consultations: Use modify_training_appointment tool
- For LISTING appointments: Use list_customer_appointments tool to get fresh data from database
- For behavioral assessments: Use behavioral_assessment tool with specific concerns
- For service information: Use training_services tool to show available programs
- Always reference actual customer and pet data from the database
- Provide positive reinforcement methods tailored to each pet's needs
...
Be encouraging, patient, and solution-focused in your training guidance! Always use the appropriate tools and reference actual customer data to provide personalized behavioral recommendations."""
)

@app.entrypoint
def max_training_agent(payload, context):
    """
    Max Training Agent with complete memory, session, and DynamoDB integration
    """
    try:
        user_input = payload.get("prompt", "Hello! How can I help you with your pet's training today?")
        customer_id = payload.get("customer_id", "")
        session_id = context.session_id or ""
        
        logger.info(f"🏋️ Max Agent - Processing request for customer: {customer_id}")
        ...
        logger.error(f"❌ Error in Max agent: {e}")
