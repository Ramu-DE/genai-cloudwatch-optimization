...
def initialize_memory(customer_id=None, session_id=None):
    ...
    # Create a single unified memory ID for Luna Nutrition
    # This memory will store all customer interactions in one place
    #target_memory_id = "LunaNutrition_Unified_Memory"
    try:
        # Create a single unified memory ID for Bella Grooming
        # This memory will store all customer interactions in one place
        # target_memory_id = "BellaGrooming_Unified_Memory"
        target_memory_id = os.getenv('BEDROCK_AGENTCORE_MEMORY_ID', 'genai_petstore_luna_nutrition_agent_memory')
    except Exception as e:
        print(f"An error occurred while retrieving environment variable '{var_name}': {e}")
        target_memory_id = 'genai_petstore_luna_nutrition_agent_memory'
    ...
    try:
        ...
        try:
            ...
            memory = memory_client.create_memory_and_wait(
                ...
                description="Unified persistent memory for Luna nutrition agent - stores all customer interactions",
                strategies=[
                    {
                        "summaryMemoryStrategy": {
                            "name": "NutritionSessionSummarizer",
                            "description": "Summarizes nutrition consultations and dietary recommendations across all customers",
                            "namespaces": ["/summaries/luna/{actorId}/{sessionId}"]  # actorId = customer_id
                        }
                    },
                    {
                        "semanticMemoryStrategy": {
                            ...
                            "namespaces": ["/knowledge/luna/{actorId}/nutrition"]  # actorId = customer_id
                        }
                    },
                    {
                        "userPreferenceMemoryStrategy": {
                            ...
                            "namespaces": ["/preferences/luna/{actorId}"]  # actorId = customer_id
                        }
                    }
                ],
...
def save_conversation_to_memory(customer_id, session_id, user_message, luna_response):
    ...
    try:
        ...
        logger.info(f"💾 User='{user_message[:30]}...', Luna='{luna_response[:30]}...'")
            ...
            for turn in recent_turns:
                for message in turn:
                    ...
                    if content:
                        ...
                        display_role = "Customer" if role.lower() == 'user' else "Luna"
